/* ---------------------- SHACK Unit (Remote) - Settings Configuration ----------------------
 *
 * This file defines settings for the SHACK Unit (Remote)
 ...
 *
 * Key differences from base settings:
 * - CONTROL_PORT_MAPPED_TO = Serial1 (RS485 communication)
 * - NO GPS_PORT (no physical GPS on Remote)
 * - Serial2 available for Nextion display (if used) on A4/A5
 * - Ethernet MAC/IP addresses specific to Remote unit
 */
...
#define SYNC_MASTER_COORDINATES_TO_SLAVE_SECS 20 // for OPTION_SYNC_MASTER_COORDINATES_TO_SLAVE - use when GPS unit is connected to slave unit and you want to synchronize the master unit coordinates to the slave GPS


/* ========== REMOTE UNIT SPECIFIC SETTINGS ========== */

// Ethernet configuration for REMOTE unit (Shack)
#define ETHERNET_MAC_ADDRESS 0xDE,0xAD,0xBE,0xEF,0xBB,0x02  // REMOTE MAC - Different from Master!
#define ETHERNET_IP_ADDRESS 192,168,100,102                  // REMOTE IP - Different from Master!
#define ETHERNET_IP_GATEWAY 192,168,100,1
...
/* ========== SERIAL PORT SETTINGS - REMOTE SPECIFIC ========== */
// IMPORTANT: Remote uses Serial1 for RS485 communication with Master unit
#define CONTROL_PORT_MAPPED_TO &Serial1    // RS485 on Serial1 (D0/D1 TX/RX)
#define CONTROL_PORT_BAUD_RATE 9600

// NO GPS on Remote unit (GPS is on Master unit at antenna)
// However, we must define GPS_PORT for K3NG compilation (even though it's not used)
// Serial2 available for Nextion display if needed (on A4/A5)
#define GPS_PORT Serial2                   // Dummy definition for compilation (not physically used)
...
// Nextion display can use Serial2 if enabled (A4/A5 available since no GPS)
